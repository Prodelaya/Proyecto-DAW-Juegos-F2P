...
| Orden | Archivo | Qué hacer | Por qué este orden |
|-------|---------|-----------|-------------------|
| 1 | `app/services/freetogame.py` | Funciones fetch_all_games() y fetch_game_detail(api_id) con requests. Manejo de errores (try/except, timeout) y pequeño rate-limit entre llamadas para no castigar la API. Devuelve lista de dicts o None | Primero el servicio, luego el seed |
...
| 2 | Templates mínimos para probar | Crear `base.html` con estructura HTML básica (sin Bootstrap aún, solo bloque content), `home.html`, `catalog.html`, `detail.html` y parciales mínimos `partials/navbar.html`, `partials/flash_messages.html`, `partials/pagination.html`. Solo para verificar que las rutas pasan datos correctos, los flash aparecen y la paginación navega | Necesarios para que render_template funcione |
...
| 2 | `app/routes/auth.py` | Blueprint auth_bp. GET+POST `/registro`: valida campos (username 3-30 chars, email válido, password mín 8 chars), verifica unicidad de email/username, hashea password, crea User, flash éxito, redirect a login. GET+POST `/login`: busca user por email, check_password, login_user(), flash, redirect a home. POST `/logout`: logout_user(), redirect a home (con CSRF) | Todas las rutas privadas dependen de esto |
...
| 4 | Error handling | Añadir manejo claro para errores 404/500 y decidir cómo responder ante fallo de CSRF (400) para que no sea una pantalla cruda sin contexto |
| 5 | Mensajes flash | Verificar que todas las acciones tienen flash message: éxito en crear/editar/eliminar, errores de validación, acceso denegado |
| 6 | Protección de rutas | Verificar que @login_required está en todas las rutas privadas. Verificar que @admin_required está en las de admin. Verificar que no puedes editar/eliminar reseña ajena (excepto admin) |
| 7 | Robustez BD | Revisar rutas POST y seeds para hacer `db.session.rollback()` si falla una operación de escritura y no dejar la sesión rota |
| 8 | Docker limpio | Probar `docker-compose down -v` + `docker-compose up --build` + ejecutar seed → todo funciona desde cero |
| 9 | Seed idempotente | Verificar que ejecutar seed_all.py dos veces no duplica datos |
